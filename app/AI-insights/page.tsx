      ...
      {!showNewInsight && step === "intro" && (
        <CommonCard className="mb-5">
          <div className="text-center mb-5">
            <div className="w-14 h-14 rounded-full bg-gradient-to-r from-[#2F80FF] to-[#5DAEFF] flex items-center justify-center mx-auto mb-3 shadow-[0_0_20px_1px_#5DAEFF]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 2xl:gap-10 2xl:max-w-7xl mx-auto mb-7">
            {steps.map((step) => {
              ...
              return (
                <div
                  key={step.id}
                  className="flex flex-col items-center text-center p-4 pb-6 rounded-3xl shadow-lg max-sm:bg-slate-50 !bg-[#ebf6ff]"
                >
...
function CircleStep({ step, activeStep }: { step: number; activeStep: number }) {
  ...
  return (
    <div
      className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-semibold shrink-0 transition-all duration-500 ${isActive
