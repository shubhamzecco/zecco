            ...
            <div className="w-full max-w-6xl mx-auto font-[Inter,system-ui]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div className="rounded-2xl border border-blue-200 bg-white p-5 sm:p-6">
                        ...
                        <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden mb-4">
                            <div style={{ width: `${mainReducer?.ai_insight?.growth_percent}%` }} className={`h-full w-[${mainReducer?.ai_insight?.growth_percent}%] bg-blue-600 rounded-full`} />
                        </div>
                        <p className="text-sm font-manrope font-semibold text-heading_text_color mb-5">
                            {mainReducer?.ai_insight?.growth_score} / 10 - {mainReducer?.ai_insight?.growth_label}
                        </p>

                        <p className="text-sm tracking-wider font-manrope font-bold text-[#64748B] mb-2">Rental Yield Potential</p>
                        <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden mb-4">
                            <div style={{ width: `${mainReducer?.ai_insight?.rental_yield_percent}%` }} className={`h-full w-[${mainReducer?.ai_insight?.rental_yield_percent}%] bg-blue-600 rounded-full`} />
                        </div>
                        <p className="text-sm font-manrope font-semibold text-heading_text_color mb-5">
                            {mainReducer?.ai_insight?.rental_yield} / 10 - {mainReducer?.ai_insight?.rental_yield_label}
                        </p>
                    ...
                    <div className="rounded-2xl border border-blue-200 bg-white p-5 sm:p-6">
                        ...
                        <div className="space-y-3 text-sm">
                            <div className="flex justify-between">
                                <span className="text-[#64748B] font-manrope font-semibold">Price per /m²:</span>
                                <span className="font-bold text-heading_text_color font-manrope">€{mainReducer?.ai_insight?.price_per_sqm} /m²</span>
                            </div>

                            <div className="flex justify-between">
                                <span className="text-[#64748B] font-manrope font-semibold">Area Average:</span>
                                <span className="font-bold text-heading_text_color font-manrope">€{mainReducer?.ai_insight?.area_avg_price_sqm} /m²</span>
                            </div>
                            ...
                            <div className="flex justify-between">
                                <span className="text-[#64748B] font-manrope font-semibold">Average Rent:</span>
                                <span className="font-bold text-heading_text_color font-manrope">€{mainReducer?.ai_insight?.average_rent_monthly}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-[#64748B] font-manrope font-semibold">Future Growth 5 Years:</span>
                                <span className="font-bold text-heading_text_color font-manrope">{mainReducer?.ai_insight?.growth_percent_5yr} %</span>
                            </div>

                            <div className="flex justify-between">
                                <span className="text-[#64748B] font-manrope font-semibold">City Distance:</span>
                                <span className="font-bold text-heading_text_color font-manrope">{mainReducer?.ai_insight?.city_distance} miles</span>
                            </div>

                            <div className="flex justify-between">
                                <span className="text-[#64748B] font-manrope font-semibold">Age of Property:</span>
                                <span className="font-bold text-heading_text_color font-manrope">{mainReducer?.ai_insight?.age_of_property} years</span>
                            </div>
                        </div>
                ...
                <div className="">
                    ...
                    <ul className="space-y-3 text-sm bg-white rounded-2xl p-6 shadow-sm">
                        ...
                        {Object.entries(mainReducer?.ai_insight?.ai_report?.ai_summary || {}).map(
                            ([key, value], i) => {
