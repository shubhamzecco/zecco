...
const SavedSearches = ({ isDashboard = false, searches }: SavedSearchesProps) => {
  ...
  return (
    <CommonCard className="max-2xl:p-5" heading={searches && searches.length > 0 ? "Saved Searches" : undefined} description={searches && searches.length > 0 ? "Recent saved searches" : undefined}>
        ...
        <div className="2xl:mt-2">
          ...
          return (
            <div
              ...
              className={`flex cursor-pointer justify-between !py-[8px] lg:!py-[2px] 2xl:!py-[9.5px] ${index !== searches.length - 1
      ...
      {searches && searches.length > 0 && (
        <button onClick={() => router.push(App_url.link.SAVED_SEARCHES)} className="mt-4 lg:mt-8 2xl:mt-4 text-[13px] font-semibold text-[#2563EB] transition hover:translate-x-1">
          Manage Searches →
        </button>
      )}
    </CommonCard>
