...
import { ChevronDown, ChevronUp, Eye, MapPin, Trash2 } from "lucide-react";
  ...
  const generateFilters = (item: any) => {
    ...
    if (item?.types?.filter(Boolean)?.length > 0) {
      filters.push({
        ...
        value: item?.types
          ?.filter(Boolean)
          ?.map((type: number) => propertyTypeMap[type] || `Type ${type}`)
          ?.join(", "),
      });
    }
    ...
    if (item?.features?.filter(Boolean)?.length > 0) {
      filters.push({
        ...
        value: item?.features
          ?.filter(Boolean)
          ?.map(
            (feature: number) => featureMap[feature] || `Feature ${feature}`,
          ),
      });
    }
    ...
    if (item?.buildFrom || item?.buildTo) {
      filters.push({
        label: "Build Size",
        value: `${item?.buildFrom || 0}m² - ${item?.buildTo || 0}m²`,
      });
    }
    ...
    <section className="mt-1">
      ...
      <div className="space-y-5">
        {mainReducer?.saved_searches?.data?.slice(0, 3)?.map((item: any) => {
          const filters = generateFilters(item);
